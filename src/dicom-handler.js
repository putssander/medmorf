...
(() => {
    ...
    function generateRobocopyFile(plan, index, mode, userSrc, userDst) {
        ...
        lines.push('}');
        lines.push('');
        lines.push('# Pause/resume support: press P to pause, any key to resume');
        lines.push('$script:paused = $false');
        lines.push('function Check-Pause {');
        lines.push('    # Check if user pressed a key');
        lines.push('    if ([Console]::KeyAvailable) {');
        lines.push('        $key = [Console]::ReadKey($true)');
        lines.push('        if ($key.Key -eq "P") {');
        lines.push('            $script:paused = $true');
        lines.push('            $stopwatch.Stop()');
        lines.push('            Write-Host "" ');
        lines.push('            Write-Host "  PAUSED -- Press any key to resume..." -ForegroundColor Yellow');
        lines.push('            Write-Log "Paused by user" "WARN"');
        lines.push('            [Console]::ReadKey($true) | Out-Null');
        lines.push('            $script:paused = $false');
        lines.push('            $stopwatch.Start()');
        lines.push('            Write-Host "  Resumed." -ForegroundColor Green');
        lines.push('            Write-Log "Resumed by user" "INFO"');
        lines.push('        }');
        lines.push('    }');
        lines.push('}');
        lines.push('');
        lines.push('# Network availability check: auto-pause when source becomes unreachable');
        lines.push('function Wait-ForNetwork([string]$testPath) {');
        lines.push('    if (Test-Path $testPath) { return }');
        lines.push('    $stopwatch.Stop()');
        lines.push('    Write-Host "" ');
        lines.push('    Write-Host "  NETWORK UNAVAILABLE -- Source path not reachable." -ForegroundColor Red');
        lines.push('    Write-Host "  Waiting for connection to restore... (Ctrl+C to abort)" -ForegroundColor Yellow');
        lines.push('    Write-Log "Network unavailable: $testPath" "WARN"');
        lines.push('    $retries = 0');
        lines.push('    while (-not (Test-Path $testPath)) {');
        lines.push('        $retries++');
        lines.push('        Start-Sleep -Seconds 5');
        lines.push('        if ($retries % 12 -eq 0) {');
        lines.push('            $mins = [math]::Round($retries * 5 / 60)');
        lines.push('            Write-Host "  Still waiting... ($mins min). Check your network connection." -ForegroundColor Yellow');
        lines.push('        }');
        lines.push('    }');
        lines.push('    $stopwatch.Start()');
        lines.push('    Write-Host "  Connection restored. Resuming copy." -ForegroundColor Green');
        lines.push('    Write-Log "Network restored after $retries retries" "INFO"');
        lines.push('}');
        lines.push('');
        ...
        lines.push('Write-Host "  Re-running is safe -- robocopy skips files already copied." -ForegroundColor Green');
        lines.push('Write-Host ""');
        lines.push('Write-Host "  Controls during copy:" -ForegroundColor DarkGray');
        lines.push('Write-Host "    P      = Pause (any key to resume)" -ForegroundColor DarkGray');
        lines.push('Write-Host "    Ctrl+C = Abort (safe to re-run later)" -ForegroundColor DarkGray');
        lines.push('Write-Host "  Network drops are detected automatically." -ForegroundColor DarkGray');
        lines.push('Write-Host ""');
        lines.push('');
        ...
        for (const [key, files] of groups) {
            ...
            lines.push('$groupNum++');
            lines.push('Check-Pause');
            lines.push('Wait-ForNetwork $SourceRoot');
        ...
        // UTF-8 BOM so PowerShell 5.1 reads encoding correctly
    ...
    function generateRobocopyReadme(stamp, scriptSrc, scriptDst, totalFiles, totalGroups, mode, index) {
        const r = [];
        r.push('=========================================================================');
        r.push('  DICOM Robocopy Sort Script -- Instructions');
        r.push('  Generated: ' + new Date().toISOString());
        r.push('  Script file: dicom-sort-' + stamp + '.ps1');
        r.push('=========================================================================');
        r.push('');
        r.push('');
        r.push('OVERVIEW');
        r.push('--------');
        r.push('This PowerShell script copies ' + totalFiles + ' DICOM files from a source');
        r.push('directory into an organized folder structure using robocopy.');
        r.push('');
        r.push('  Patients:     ' + index.scanInfo.patientCount);
        r.push('  Studies:      ' + index.scanInfo.studyCount);
        r.push('  Series:       ' + index.scanInfo.seriesCount);
        r.push('  Files:        ' + totalFiles);
        r.push('  Folder groups: ' + totalGroups);
        r.push('  Sort mode:    ' + mode);
        r.push('');
        r.push('  IMPORTANT: Files are only COPIED, never moved or deleted.');
        r.push('  Source files are never modified. It is safe to re-run the script.');
        r.push('');
        r.push('');
        r.push('=========================================================================');
        r.push('  STEP 1: CONFIGURE PATHS');
        r.push('=========================================================================');
        r.push('');
        r.push('Open the .ps1 file in a text editor (Notepad, VS Code, Notepad++).');
        r.push('At the top you will find two path variables:');
        r.push('');
        r.push('  $SourceRoot = "' + scriptSrc + '"');
        r.push('  $DestRoot   = "' + scriptDst + '"');
        r.push('');
        r.push('SOURCE PATH ($SourceRoot)');
        r.push('  This must point to the folder that was indexed in MedMod.');
        r.push('  The browser only sees the folder name, not the full path,');
        r.push('  so verify this is correct.');
        r.push('');
        r.push('  Examples:');
        r.push('    Local:   C:\\Data\\DICOM\\PatientExport');
        r.push('    UNC:     \\\\server.hospital.nl\\share\\DICOM\\Export');
        r.push('    Mapped:  Z:\\DICOM\\Export');
        r.push('');
        r.push('DESTINATION PATH ($DestRoot)');
        r.push('  Where the organized folder structure will be created.');
        r.push('  This directory will be created if it does not exist.');
        r.push('');
        r.push('  Examples:');
        r.push('    Local:   D:\\DICOM_Sorted');
        r.push('    UNC:     \\\\server.hospital.nl\\output\\Sorted');
        r.push('    Mapped:  Z:\\Output\\Sorted');
        r.push('');
        r.push('  TIP: UNC paths (\\\\server\\share) are faster than mounted/mapped');
        r.push('  drives for robocopy. Robocopy talks SMB directly without the');
        r.push('  overhead of the Windows drive-mapping layer.');
        r.push('');
        r.push('  WARNING: Source and destination must be different folders.');
        r.push('  The script will refuse to run if they are the same.');
        r.push('');
        r.push('');
        r.push('=========================================================================');
        r.push('  STEP 2: SET PERMISSIONS TO RUN THE SCRIPT');
        r.push('=========================================================================');
        r.push('');
        r.push('Windows blocks downloaded scripts by default. You have several options:');
        r.push('');
        r.push('');
        r.push('OPTION A: Unblock this specific file (recommended, safest)');
        r.push('-----------------------------------------------------------');
        r.push('  1. Right-click the .ps1 file in File Explorer');
        r.push('  2. Click "Properties"');
        r.push('  3. At the bottom, check the box "Unblock"');
        r.push('  4. Click OK');
        r.push('');
        r.push('  Or run this in PowerShell:');
        r.push('    Unblock-File -Path ".\\dicom-sort-' + stamp + '.ps1"');
        r.push('');
        r.push('');
        r.push('OPTION B: Allow scripts for this session only');
        r.push('---------------------------------------------');
        r.push('  Open PowerShell and run:');
        r.push('    Set-ExecutionPolicy -ExecutionPolicy Bypass -Scope Process');
        r.push('');
        r.push('  This allows scripts only in the current window.');
        r.push('  Close the window and the permission is gone.');
        r.push('');
        r.push('');
        r.push('OPTION C: Run directly with bypass (one-liner)');
        r.push('----------------------------------------------');
        r.push('  powershell -ExecutionPolicy Bypass -File ".\\dicom-sort-' + stamp + '.ps1"');
        r.push('');
        r.push('');
        r.push('NETWORK / FILE PERMISSIONS');
        r.push('--------------------------');
        r.push('  The account running the script needs:');
        r.push('    - READ access to the source directory (and all subdirectories)');
        r.push('    - WRITE access to the destination directory');
        r.push('');
        r.push('  For UNC paths, make sure:');
        r.push('    - You can browse to \\\\server\\share in File Explorer');
        r.push('    - Your Windows session has valid credentials for the share');
        r.push('    - If prompted, authenticate first in Explorer before running');
        r.push('');
        r.push('  If you get "Access Denied" errors:');
        r.push('    1. Open File Explorer and navigate to the UNC source path');
        r.push('    2. Enter credentials if prompted (check "Remember me")');
        r.push('    3. Try the script again');
        r.push('');
        r.push('  To pre-authenticate a UNC share from command line:');
        r.push('    net use \\\\server\\share /user:DOMAIN\\username');
        r.push('');
        r.push('');
        r.push('=========================================================================');
        r.push('  STEP 3: RUN THE SCRIPT');
        r.push('=========================================================================');
        r.push('');
        r.push('  1. Open PowerShell (regular user is fine, no admin needed)');
        r.push('  2. Navigate to where you saved the script:');
        r.push('       cd "$HOME\\Downloads"');
        r.push('  3. Run:');
        r.push('       .\\dicom-sort-' + stamp + '.ps1');
        r.push('');
        r.push('  The script will:');
        r.push('    - Show a summary of what it will do');
        r.push('    - Ask for confirmation before copying anything');
        r.push('    - Show progress with file counts and elapsed time');
        r.push('    - Write a timestamped log file in the same folder');
        r.push('    - Report errors clearly at the end');
        r.push('');
        r.push('');
        r.push('=========================================================================');
        r.push('  TROUBLESHOOTING');
        r.push('=========================================================================');
        r.push('');
        r.push('SCRIPT WON\'T RUN / "not recognized"');
        r.push('  Make sure you are in the correct directory:');
        r.push('    cd "$HOME\\Downloads"');
        r.push('    dir dicom-sort-*.ps1');
        r.push('');
        r.push('"... is not digitally signed"');
        r.push('  Use Option A, B, or C from the permissions section above.');
        r.push('');
        r.push('"Source path not found"');
        r.push('  - Check that the network share is connected');
        r.push('  - Try navigating to the path in File Explorer first');
        r.push('  - Verify the exact spelling (no trailing backslash)');
        r.push('');
        r.push('"Access denied" on destination');
        r.push('  - Check you have write access to the destination folder');
        r.push('  - Try creating a test file there manually first');
        r.push('  - For network paths, authenticate via Explorer first');
        r.push('');
        r.push('SOME FILES FAILED');
        r.push('  - Check the log file (dicom-sort-log-*.txt) for details');
        r.push('  - Re-run the script -- robocopy skips already-copied files');
        r.push('  - Common causes: file locked, intermittent network, permission');
        r.push('');
        r.push('SCRIPT IS SLOW');
        r.push('  - Use UNC paths instead of mapped drives');
        r.push('  - Check network bandwidth (large CT series can be gigabytes)');
        r.push('  - Robocopy options can be tuned in the $RobocopyOpts section');
        r.push('  - Add /MT:8 to $RobocopyOpts for multi-threaded copy (8 threads)');
        r.push('    Note: /MT is only available on Windows 7+ and may increase');
        r.push('    network load. Start with /MT:4 on shared infrastructure.');
        r.push('');
        r.push('WANT TO VERIFY THE COPY');
        r.push('  Re-run the script. Robocopy will report "0 copied" if all files');
        r.push('  are already present and identical. Any mismatches will be re-copied.');
        r.push('');
        r.push('');
        r.push('=========================================================================');
        r.push('  ROBOCOPY OPTIONS REFERENCE');
        r.push('=========================================================================');
        r.push('');
        r.push('The script uses these defaults (editable in the $RobocopyOpts section):');
        r.push('');
        r.push('  /COPY:DAT   Copy Data, Attributes, Timestamps');
        r.push('  /R:3        Retry failed files 3 times');
        r.push('  /W:5        Wait 5 seconds between retries');
        r.push('  /NP         No per-file % progress (cleaner output)');
        r.push('  /NDL        No directory list in output');
        r.push('  /NFL        No file list in output (script logs itself)');
        r.push('  /NJH        No job header');
        r.push('  /NJS        No job summary');
        r.push('');
        r.push('Useful additions you can add:');
        r.push('  /MT:8       Multi-threaded copy (8 threads, faster on large sets)');
        r.push('  /LOG:file   Write robocopy output to a log file');
        r.push('  /Z          Restartable mode (resumes interrupted large files)');
        r.push('  /COPY:DATS  Also copy security info (needs elevated permissions)');
        r.push('');
        r.push('');
        r.push('=========================================================================');
        r.push('  SAFETY NOTES');
        r.push('=========================================================================');
        r.push('');
        r.push('  - The script ONLY copies files. It never deletes, moves, or');
        r.push('    modifies source files.');
        r.push('  - Source and destination must be different directories.');
        r.push('  - The script asks for confirmation before any files are copied.');
        r.push('  - Every run is logged with a unique timestamp.');
        r.push('  - You can re-run safely to retry failed files or verify copies.');
        r.push('  - Robocopy exit codes 0-3 are normal. Code 8+ indicates errors.');
        r.push('');
        r.push('=========================================================================');
        return r.join('\r\n');
    }
