...
// --summary-js writes the compact module the app reads (model cards, Benchmark tab).
...
if (summaryJs) {
    ...
    for (const set of setKeys) {
        ...
        const put = (r, kind) => {
            ...
            m.results[set] = { status: 'pass', recall: +a.recall.toFixed(3), precision: +a.precision.toFixed(3), quasi: a.quasi == null ? null : +a.quasi.toFixed(3), groups: a.groups, over: a.over == null ? null : +a.over.toFixed(3), inferMsPerDoc: Math.round(r.inferMsPerDoc || 0), heapMB: r.heapPeakDeltaMB == null ? null : Math.round(r.heapPeakDeltaMB) };
        };
        ...
        for (const n of ners.filter(r => r.status === 'pass')) for (const l of llms.filter(r => r.status === 'pass')) {
            ...
            u.results[set] = { recall: +a.recall.toFixed(3), precision: +a.precision.toFixed(3), quasi: a.quasi == null ? null : +a.quasi.toFixed(3), groups: a.groups, over: a.over == null ? null : +a.over.toFixed(3), docs: ids.length };
        }
    }
