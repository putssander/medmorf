...
- **Shared anonymize logic lives in pure modules:** text chunking in `src/text-chunking.js` and detector/LLM output filters in `src/anonymize-filters.js`. Both `anonymize-handler.js` and `benchmark-handler.js` import them so the benchmark measures exactly what the app does — change behaviour there, never by re-implementing it in one caller. Anonymize fixtures (`tests/fixtures/anonymize*.json`) use `{ types, documents: [{ id, language, text, pii: [{ text, type }], allowed }] }`; every `pii.text` must be a verbatim substring of `text`. The MedDeID sample is generated by `tools/convert-meddeid.mjs` (CC BY 4.0, keep the attribution).
