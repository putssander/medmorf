...
- **Shared anonymize logic lives in pure modules:** text chunking in `src/text-chunking.js`, detector/LLM output filters in `src/anonymize-filters.js`, and streamed LLM extraction with the repetition guard + salvage parser in `src/llm-extract.js`. Both `anonymize-handler.js` and `benchmark-handler.js` import them so the benchmark measures exactly what the app does — change behaviour there, never by re-implementing it in one caller. `src/benchmark-published.js` is **generated** by `tools/benchmark-report.mjs --summary-js` from a raw export in `tests/benchmark-results/` — never hand-edit it; rerun the tool (see README → Model Benchmark). The public source of truth is `https://medmorf.com/#benchmark`: keep measured scores on that page and in the generated report, not duplicated in README or tool panels. Generate F1 as the mean of per-document F1, matching the runtime aggregation. Anonymize fixtures (`tests/fixtures/anonymize*.json`) use `{ types, documents: [{ id, language, text, pii: [{ text, type }], allowed }] }`; every `pii.text` must be a verbatim substring of `text`. The MedDeID sample is generated by `tools/convert-meddeid.mjs` (CC BY 4.0, keep the attribution).
